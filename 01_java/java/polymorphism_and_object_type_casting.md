...
상위 클래스 또는 인터페이스의 형변환을 허용한다.

`상위 타입`에 모든 `하위 객체` 대입  ok
...
하위 객체는 상위 객체의 특징과 기능을 상속받기 때문에 부모와 동일하게 취급될 수 있다는 개념이다.

`하위 클래스`는 `상위 클래스`으로 자동 형 변환이 가능하다!
...
:mushroom:  바로 상위의 객체가 아니더라도 상속 계층에서 **상위 타입**이라면 자동 형 변환이 일어날 수 있다.

<br>

:mushroom: **자동 형 변환이 일어난 후에는 ?**

> 상위 타입으로 자동 형 변환이 된 이후에는 상위 클래스에 선언된 필드와 메소드만 접근할 수 있다.
>
> <mark>**단, 예외적으로 메소드가 하위 클래스에서 Overriding 되었다면 자식 클래스의 메소드가 대신 호출된다!**</mark>
...
:mushroom: **굳이 상위 타입으로 변환해서 사용하는 이유는 ?**
...
> <br>
>
> 상위 클래스를 상속하는 하위 클래스는 상위 클래스가 가지고 있는 필드와 메소드를 가지고 있으니 사용 방법이 동일하고, 하위 클래스는 상위 클래스의 메소드를 오버라이딩(재정의)해서 메소드의 실행 내용을 변경함으로써 더 우수한 실행 결과가 나오게 할 수 있다. 그리고 하위 타입을 상위 타입으로 변환할 수 있다.
...
매개 변수의 타입이 클래스일 경우, 해당 클래스의 객체뿐만 아니라 하위 객체까지도 매개값으로 사용할 수 있다!

매개값으로 어떤 하위 객체가 제공되느냐에 따라 메소드의 실행 결과는 다양해질 수 있다. 
...
`상위 타입`을 `하위 타입`으로 변환하는 것을 말한다.

하지만 하위 타입이 상위 타입으로 변환한 상태(자동 형 변환)일 경우에만 가능하다.

즉, `하위 타입`이 `상위 타입`으로 자동 변환하고 나서 다시 `하위 타입`으로 변환할 때 강제 형 변환을 사용한다.

<br>

:mushroom: **상위 변수가 참조하는 객체가 상위 객체인지 하위 객체인지 확인하는 방법은 ?**
