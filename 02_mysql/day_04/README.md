...
- `UNION` : 두 쿼리의 결과를 행으로 합치는 것
  - `UNION`  : 중복된 열은 제거되고 데이터가 정렬되어 나옴
  - `UNION ALL` : 중복된 열까지 모두 출력

- `NOT IN` : 첫 번째 쿼리 결과 중, 두 번째 쿼리에 해당하는 것을 제외함
- `IN` : NOT IN과 반대로 첫 번째 쿼리의 결과 중, 두 번째 쿼리에 해당되는 것만 조회
...
- 특정 데이터 입력 시 무조건적으로 입력되는 것이 아닌, 어떤 조건을 만족했을 때 입력되도록 제한 o
